...
import { isRequestSuccessful, sanitizeData } from "@/../../lib/helpers";
import jwtAxios from "@/../../lib/services/axios";
...
export const putDataApi = (
  ...
  console.log("updating a record", payload);
  const { fetchStart, fetchSuccess } = infoViewContext;
  return new Promise((resolve, reject) => {
    if (!isHideLoader) fetchStart();
    jwtAxios
...
export const patchDataApiSts = (
  ...
  console.log("updating a record", payload);
  const { fetchStart, fetchSuccess } = infoViewContext;
  console.log(url + payload.CampaignIds[0], payload.archived);
  return new Promise((resolve, reject) => {
    if (!isHideLoader) fetchStart();
    for (let i = 0; i < payload.CampaignIds.length; i++) {
      console.log("payload", payload.CampaignIds.length)
      jwtAxios
        .patch(url + payload.CampaignIds[i], !payload.archived)
        .then((data) => {
          return handleApiResponse(url, fetchSuccess, data, resolve, reject);
        })
        .catch((error) => {
...
export const deleteDataApi = (
  ...
  return new Promise((resolve, reject) => {
    if (!isHideLoader) fetchStart();
    for (let i = 0; i < payload.recordIds?.length; i++) {
       console.log("Deleting", payload)
       console.log("length: ", payload.recordIds?.length)
    jwtAxios
      .delete(url + payload.recordIds[i])
      .then((data) => {
        return handleApiResponse(url, fetchSuccess, data, resolve, reject);
      })
      .catch((error) => {
