...
export const putDataApi = (
  ...
  return new Promise((resolve, reject) => {
    if (!isHideLoader) fetchStart();
    jwtAxios
      .put(url + `${id}`, sanitizeData(payload))
      .then((data) => {
        return handleApiResponse(url, fetchSuccess, data, resolve, reject);
      })
      .catch((error) => {
...
export const patchDataApi = (
  ...
  return new Promise((resolve, reject) => {
    ...
    for (let i = 0; i < payload.ids.length; i++) {
      jwtAxios
        .patch(url + payload.ids[i], body)
        .then((data) => {
          return handleApiResponse(url + payload.ids[i], fetchSuccess, data, resolve, reject);
        })
        .catch((error) => {
          return handleAPIError(url + payload.ids[i], fetchSuccess, error, reject);
        });
    }
    return Promise.resolve();
  });
